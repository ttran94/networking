...
class Packet:
	...
	def calculate_udp_checksum(self):
		...
		dst_addr = socket.inet_aton(self.dst_addr)
		protocol = socket.IPPROTO_UDP
		udp_fragment = src_addr + dst_addr + struct.pack('!HHHLLBB',
			self.udp_src_port,
			self.dst_port,
			self.udp_length,
			self.udp_seq,
	...
	def assemble_packet(self):
		src_addr = socket.inet_aton(self.src_addr)
		dst_addr = socket.inet_aton(self.dst_addr)
		self.raw = src_addr + dst_addr + struct.pack('!BBHHHHHHHHLLBB',
			self.zeroes,
			self.protocol,
			self.src_port,
			self.dst_port,
...
def verify_udp_checksum(packet_udp, data):
	...
	(udp_src_port, udp_dst_port, udp_length, udp_checksum, udp_seq, udp_ack_seq, control, data_len) = struct.unpack_from("!HHHHLLBB", packet_udp, offset=18)
	udp_fragment = struct.pack('!LLHHHLLHBB',
		src_addr,
		dst_addr,
		udp_src_port,
		udp_dst_port,
...
if __name__ == '__main__':
	...
	(udp_src_port, udp_dst_port, udp_length, udp_checksum, udp_seq, udp_ack_seq, control, data_len) = struct.unpack_from("!HHHHLLBB", packet.raw, offset=18)
