...
class Ringo:
    def __init__(self, flag, local_port, poc_host, poc_port, n):
        ...
        self.rtt_matrix = {}
        self.activeRingos = set()
        self.offlineRingos = set()
            ...
            if "Peer Discovery" in data:
                for peer in data.split("/")[1].split(","):
                    ...
                    if addr_of_peer != (self.local_host, self.local_port) and addr_of_peer not in self.peers:
                        ...
                        self.activeRingos.add(addr_of_peer)
        ...
        while (1):
            ...
            for peer in self.offlineRingos:
                try:
                    ...
                    self.activeRingos.remove(peer)
                    self.offlineRingos.add(peer)
        return

    def check_offline_ringos (self):
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        msg = "Keep Alive"
        while (1):
            time.sleep(15)
